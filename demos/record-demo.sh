...
CAST_FILE="$DEMO_DIR/demos/quick-start.cast"
GIF_FILE="$DEMO_DIR/demos/quick-start.gif"
...
cp "$GIF_FILE" "$SOURCE_DIR/demos/" 2>/dev/null || echo "   (no .gif output)"
cp "$CAST_FILE" "$SOURCE_DIR/demos/" 2>/dev/null || echo "   (no .cast output)"
